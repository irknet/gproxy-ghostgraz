					 ...
					 size_t exe_info_size, uint32_t* version, int platform)
{
	...
	base = get_basename(file_name);
        ...
        size_t alloc_len = strlen(file_name) + 1;
        wchar_t* w_file_name = new wchar_t[alloc_len];
        mbstowcs(w_file_name, file_name, alloc_len);
		...
		case BNCSUTIL_PLATFORM_X86:
                {
                    ...
                    size_t alloc_len = strlen(backslash) + 1;
                    wchar_t* w_backslash = new wchar_t[alloc_len];
                    mbstowcs(w_backslash, backslash, alloc_len);

			infoSize = GetFileVersionInfoSize(w_file_name, &bytesRead);
			if (infoSize == 0)
				return 0;
			...
			if (buf == NULL)
				return 0;
			if (GetFileVersionInfo(w_file_name, NULL, infoSize, buf) == FALSE)
				return 0;
			if (!VerQueryValue(buf, w_backslash, (LPVOID*) &ffi, (PUINT) &infoSize))
				return 0;
			...
			*version =
			...
			bncsutil_debug_message_a("%s version = %d.%d.%d.%d (0x%08X)",
				...
				*version);
#endif
			VirtualFree(buf, (unsigned long) 0, MEM_RELEASE);
#else
...
#ifdef MOS_WINDOWS
	hFile = CreateFile(w_file_name, GENERIC_READ, FILE_SHARE_READ, NULL,
