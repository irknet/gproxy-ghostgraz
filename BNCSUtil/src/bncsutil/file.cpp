...
file_t file_open(const char* filename, unsigned int mode)
{
	...
	if (mode & FILE_READ) {
        ...
        size_t alloc_len = strlen(filename) + 1;
        wchar_t* w_filename = new wchar_t[alloc_len];
        mbstowcs(w_filename, filename, alloc_len);

	file = CreateFile(w_filename, access, share_mode, NULL, open_mode,
...
void file_close(file_t file)
{
	...
	for (it = file->mappings.begin(); it != file->mappings.end(); it++) {
		UnmapViewOfFile(const_cast<void*>((*it).first));
...
void file_unmap(file_t file, const void* base)
{
	...
	UnmapViewOfFile(const_cast<void*>(base));
	CloseHandle(mapping);
