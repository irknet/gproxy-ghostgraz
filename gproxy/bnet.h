...
public:
	...
	//phy extract an past from private
	CBNETProtocol *m_Protocol;	// battle.net protocol
	uint32_t m_repeat_display_games;
	string m_UserName;								// battle.net username..private
...
public:
	...
	string GetFriend( int i ) {return friends[i];}
...
//	void dl();
	...
	{ 
		int i;
		bool check=false;
		int temp=m_SearchGameCount %10;//max 10 games in the list
		
		
		for(i=0;i<=m_SearchGameCount;i++)
		{
			if(m_SearchGameName[i]==nSearchGameName)
				check=true;
		}
		if(!check)
		{
			m_SearchGameName[m_SearchGameCount] = nSearchGameName; 
			m_SearchGameNameTime[m_SearchGameCount] = GetTime( ); 
			if (m_SearchGameCount<=9)
			m_SearchGameCount++;
		}
		}*/
