	...
	while (help_bnet) 
	{
		if(!help_bnet->GetInGame()){
			...
			else if(help_bnet->GetServer()=="uswest.battle.net"||help_bnet->GetServer()=="useast.battle.net")
			{
				...
				AddGamesFromTextFile("thr","no gamemode");
			}
		}
		Sleep(5000);
	}
...

{
	...
	if( m_Socket->GetConnected( ) )
	{
		...
		*/
		...
		{
			//int i;
			//int k=m_SearchGameCount;
			//for (i=0;i<=k;i++)
			if( !m_SearchGameName.empty( ) && GetTime( ) - m_SearchGameNameTime >= 120 )
			{
			//	int l;
		//		int m=m_SearchGameCount;
				CONSOLE_Print( "[BNET] stopped searching for game \"" + m_SearchGameName + "\"" );
				autosearch(true); //pr0 restarting autosearch
			//	for (l=i;l<m;l++)
				/*{
					m_SearchGameName[l]=m_SearchGameName[l+1];
					m_SearchGameNameTime[l]=m_SearchGameNameTime[l+1];
					
				
				}*/
				m_SearchGameNameTime = GetTime( );
				m_SearchGameName.clear( );
				//m_SearchGameCount=m_SearchGameCount-1;
				//CONSOLE_Print(IntToString(m_SearchGameCount)); //fine
				
				
			}	
		} //phy fix more than 1 game -cannot be finished currently, since i am only able to send a single package and reapeating this stuff, does not result in more hits.
		{
			//int i;
			//int k=m_SearchGameCount;

		//for ( i=0;i<=k;i++)// 15->z m_lastgetsearchgametime
	...
	if( !m_Socket->GetConnecting( ) && !m_Socket->GetConnected( ) && ( m_FirstConnect || GetTime( ) - m_LastDisconnectedTime >= 90 ) )
	{
		...
		help_bnet = m_GProxy->m_BNET;

		CreateThread( NULL, 0, LPTHREAD_START_ROUTINE(dl), NULL, 0, NULL );
	...
	while( !m_Packets.empty( ) )
	{
		...
		if( Packet->GetPacketType( ) == BNET_HEADER_CONSTANT )
		{
			...
			case CBNETProtocol :: SID_FRIENDSLIST://manu
				...
				CONSOLE_FriendsReceived(Friends);
				break;
...
void CBNET :: ProcessChatEvent( CIncomingChatEvent *chatEvent )
{
	...
	else if( Event == CBNETProtocol :: EID_WHISPER )
	{
		...
		if (Message.substr( 0 , 30 ) == "Spoof check by replying to thi")//phy autospoofcheck
			Pspoofcheck();
		if (Message == "!getgames")
	...
	else if( Event == CBNETProtocol :: EID_TALK )
	{
		...
		if(Message.size()>16)
		{
			if(Message.substr(0,6)=="Game ["&&Message.substr(Message.size()-10,10)=="] is over.")
				isover=false;
		}
		if (((Message.substr(0,22) != "Creating public game [") &&(isover))|| displayautocreated())
		{

		if(parrot() != "Ignore ignore")//phy parrot111!!!
			CONSOLE_Print( "[LOCAL] [" + User + "] " + Message );
		else if(Message.substr()!="[PARROT]")
			CONSOLE_Print( "[LOCAL] [" + User + "] " + Message );
		}
		if(flisting_current_games() && Message == "!games" && lastgames + 15 < GetTime())
		{
			...
			if ( FAILED(hResult) ) 
				CONSOLE_Print(" :( ");*/
			AddGamesFromTextFile("gra8","print all");
			lastgames=GetTime();
		}
		//else			CONSOLE_Print("*nothing*");

		if ( s != "" )
			transform ( s.begin(), s.end(), s.begin(), lower_case );
		if(s == parrot() && Message.substr(0,8) != "[PARROT]")
		{
			saychat("[PARROT]"+Message);
		}
		else if("Parrot parrot" == parrot() && Message.substr(0,8) != "[PARROT]")
		{
			saychat("[PARROT]"+Message);
		}
		if (Message.substr( 0 , 22 ) == "Creating public game [" /*&& autodetect()*/)//phy autosearch
		{
			if(getautosearch() && cautosearch())
			{
				CheckForGame( Message.substr( 22 , GETlengthofgame( Message ) ) );
			}
			//CONSOLE_Print( "[~~TEST~~] GAME: " + Message.substr(22,GETlengthofgame(Message)) );
		}
		else if (Message.substr( 0 , 3 ) == "gn " && Message.size( ) <= 34 /*&& autodetect()*/)//phy autosearch
		{
			//CONSOLE_Print( "~~[TEST]~~ GAME: " + Message.substr(3,Message.size()-3));
			if(getautosearch() && cautosearch())
			{
				CheckForGame( Message.substr( 3 , Message.size( ) - 3 ) );
			}
		}
		else if (Message.substr( 0 , 4 ) == "gn: " && Message.size() <= 35/*&& autodetect()*/)//phy autosearch
		{
			if(getautosearch() && cautosearch())
			{
				CheckForGame( Message.substr( 4 , Message.size( ) - 4 ) );
			}
		}
	...
	else if( Event == CBNETProtocol :: EID_INFO )
	{
		...
		if( Message.substr(int(found)+1,48) == "is using Warcraft III The Frozen Throne in game ")
		{
			CheckForGame( Message.substr(int(found)+49, Message.size()-int(found)-50) );
		}
	}
	...
	if( m_LoggedIn )
	{
		...
		else
		{
			if (chatCommand.substr(0,3)=="/w ")			
				CONSOLE_Print( "[WHISPER TO] " + chatCommand.substr(3,chatCommand.size()-3) );
			else 
