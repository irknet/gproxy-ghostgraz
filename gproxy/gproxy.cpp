...
/* Phytons variables */
bool vShallCreate = false;					// Used for waitgame
bool vShallCreateQuiet = false;				// Used for waitgame
string botprefix = "GhostGraz";				// Used for waitgame
string privategamename = "my gamename";		// Used for waitgame and priv
...
//string playerNames[12];					// Player name at playerID @depricated
CPlayer players[12];						// ^ this class is used instead ^
...
const int StrToInt(const std::string& s)// Phyton stats
{
    return std::atoi(s.c_str());
}

// Phyton dl3
string getfromtextfile(string input, string want)
{
    if (want == "botname")
    {
...
void CONSOLE_Draw()
{
    ...
    if (gInputWindowChanged)
    {
        ...
        // Phytong german_languagesupport
...
int main(int argc, char **argv)
{
    ...
    string Server;
    string Username;
    string cfilter = ""; // Phyton public
    string cpublic = ""; // Phyton filter
    bool autodetecttemp = false; //autodetect phy
    bool german_languagesupport = false; // Phyton language
    ...
    else
    {
        ...
        if (CFG.Exists("privategamename")) // Phyton priv waitgame
		{
            privategamename = CFG.GetString("privategamename", string());
		}
        
		if (CFG.Exists("botprefix")) // Phyton waitgame
		{
            botprefix = CFG.GetString("botprefix", string());
		}
		
		if (CFG.Exists("autodetect")) // Phyton autodetect
		{
		...
		if (CFG.Exists("public")) // Phyton public
		{
		...
		if (casearch = false && CFG.Exists("autodetect")) //pr0 autosearch
		{
		...
		if (CFG.Exists("filter")) // Phyton filter
        {
        ...
        if (CFG.Exists("autostart"))
        {
            if (CFG.GetString("autostart", string()) == "off" || CFG.GetString("autostart", string()) == "false")
            {
                autostart = false;
            }
        }
        ...
        while (c != ERR)
        {
                        ...
                        if (user.size() != 0)
                        {
                            if (pos.bstate == BUTTON1_CLICKED || pos.bstate == BUTTON1_RELEASED)
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        if (gGProxy->m_GameStarted)
                                        {
                                            gGProxy->SendAllMessage("!stats " + channelList[y]);
                                        }
                                        else
                                        {
                                            gGProxy->SendLobbyMessage("!stats " + channelList[y]);
                                        }
                                    }
                                }
                            }
                            else if (pos.bstate == BUTTON3_CLICKED || pos.bstate == BUTTON3_RELEASED)
                            {
                                ...
                                else
                                {
                                    if (gGProxy->m_GameStarted)
                                    {
                                        gGProxy->SendAllMessage("!statsdota " + channelList[y]);
                                    }
                                    else
                                    {
                                        gGProxy->SendLobbyMessage("!statsdota " + channelList[y]);
                                    }
                                }
                            }
                        }
...
#endif
            {
                ...
                if (Command == "/commands")
                {
                    ...
                    CONSOLE_Print("   /pgn <privgamename> : sets privategamename temporary to <privgamename> (Alias: /privategamename)", false); // Phyton ofc i have added the new commands.
                ...
                else if (Command == "/listing_current_games off" || Command == "/lcg off")
                {
                    ...
                    /*else if ( Command.substr(0,7) == "/stats ") // Phyton using a dumpfile of the graz-database..
                    {
                ...
                else if (Command.substr(0, 11) == "/botprefix ")// Phyton
                {
                ...
                else if (Command.substr(0, 9) == "/waitgame" && botprefix != "")// Phyton waitgame
                {
                    if (Command == "/waitgame")
                    {
                        //if(!(botprefix.substr(0,botprefix.size())==botprefix))
                ...
                {
                    CONSOLE_Print("[GPROXY] Customized GProxy++ Version " + gGProxy->m_Version);
...
CGProxy::CGProxy(bool nTFT, string nWar3Path, string nCDKeyROC, string nCDKeyTFT, string nServer, string nUsername, string nPassword, string nChannel, uint32_t nWar3Version, uint16_t nPort, BYTEARRAY nEXEVersion, BYTEARRAY nEXEVersionHash, string nPasswordHashType, string cpublic, string cfilter, bool german_languagesupport, bool casearch, bool temp_displayautocreated, bool listing_current_games, int channelWidth)
{
    ...
    help_gproxy = m_BNET->m_GProxy; // Phyton
    m_PlaySound = true; // manu

    if (FAILED(CoInitialize(NULL))) // Needed for URLDownloadToFile
        CONSOLE_Print("[Phyton] Error with dl");

    CONSOLE_Print("[GPROXY] Customized GProxy++ Version " + m_Version);
}
        ...
        if (GetTime() - m_LastRefreshTime >= 2)
        {
            for (vector<CIncomingGameHost *> ::iterator i = m_Games.begin(); i != m_Games.end();)
            {
                ...
                if ((*i)->GetMapWidth() == 1984 && (*i)->GetMapHeight() == 1984)
                {
                    ...
                    else if (tempstr.substr(0, 9) == "GhostGraz")// Phyton ghostgrazgame orange
    ...
    while (!m_RemotePackets.empty())
    {
        ...
        if (Packet->GetPacketType() == W3GS_HEADER_CONSTANT)
        {
            if (Packet->GetID() == CGameProtocol::W3GS_CHAT_FROM_HOST)
            {
                ...
                {
                    string playerName;
                    if (players[FromPID].GetName().substr(0, 2) == "|c" || players[FromPID].GetName().substr(0, 2) == "|C") // Colored name
                        playerName = string() + char(6) + char(14) + players[FromPID].GetName().substr(10) + char(6) + char(7);
                    else
                        playerName = players[FromPID].GetName();
                    ...
                    else // GameStarted
                    {
                        ...
                        else if (ExtraFlags[0] >= 3) // 3+ are private messages
                        {
                            ...
                            CONSOLE_Print(string() + char(6) + char(11) + "[PRIVATE][FROM][" + players[FromPID].GetName() + "] " + MessageString);
                        }
                    }
            ...
            if (Packet->GetID() == CGameProtocol::W3GS_PLAYERLEAVE_OTHERS)
            {
                ...
                if (gGProxy->m_GameStarted)
                {
                    ...
                    CONSOLE_Print("[ALL] " + players[((int) Data[4])].GetName() + " has left the game.");
                }
                else
                    CONSOLE_Print("[LOBBY] " + players[((int) Data[4])].GetName() + " has left the game.");
            ...
            if (Packet->GetID() == CGameProtocol::W3GS_SLOTINFOJOIN)
            {
                ...
                if (Data.size() >= 6)
                {
                    ...
                    if (Data.size() >= 7 + SlotInfoSize)
                    {
                        ...
                        players[((int) m_ChatPID)].SetName(m_Username);
                    }
                ...
                for (int j = 0; j < totalSlots; j++)
                {
                    for (vector<CIncomingSlots *> ::iterator it = slots.begin(); it != slots.end(); it++)
                    {
                        if (((*it)->GetColor() == ((int) Data[i + 5])))
                        {
                            ...
                            (*it)->SetName(players[((int) Data[i])].GetName());
            ...
            else if (Packet->GetID() == CGameProtocol::W3GS_GAMELOADED_OTHERS)
            {
                ...
                else
                {
                    ...
                    finishedLoading << players[((int) Packet->GetData()[4])].GetName();
