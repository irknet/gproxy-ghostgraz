...
CBNET::CBNET (CGProxy *nGProxy, MainGUI *p_mainGUI, string nServer, string nBNLSServer, uint16_t nBNLSPort, uint32_t nBNLSWardenCookie, string nCDKeyROC, string nCDKeyTFT, string nCountryAbbrev, string nCountry, string nUserName, string nUserPassword, string nFirstChannel, unsigned char nWar3Version, BYTEARRAY nEXEVersion, BYTEARRAY nEXEVersionHash, string nPasswordHashType, uint32_t nMaxMessageLength)
{
    m_GProxy = nGProxy;
    mainGUI = p_mainGUI;
        ...
        if (Packet->GetPacketType() == BNET_HEADER_CONSTANT)
        {
            switch (Packet->GetID())
            {
                ...
                case CBNETProtocol::SID_AUTH_INFO:
                    ...
                    {
                        if (m_BNCSUtil->HELP_SID_AUTH_CHECK(m_GProxy->m_TFT, m_GProxy->GetWar3Path(), m_CDKeyROC, m_CDKeyTFT, m_Protocol->GetValueStringFormulaString(), m_Protocol->GetIX86VerFileNameString(), m_Protocol->GetClientToken(), m_Protocol->GetServerToken()))
                        {
...
void CBNET::ProcessChatEvent (CIncomingChatEvent * chatEvent)
{
    ...
    else if (Event == CBNETProtocol::EID_TALK)
    {
        ...
        if (s == QString::fromStdString(m_GProxy->GetParrot()) && !Message.mid(0, 8).startsWith("[PARROT]"))
        {
