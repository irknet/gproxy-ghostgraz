...
public:
    ...
    uint32_t m_repeat_display_games;
    string m_UserName;
...
private:
    ...
    string m_SearchGameName; //phy changed all m_SearchGameName to array
    int m_SearchGameCount; //phy [20]
    uint32_t m_SearchGameNameTime;
    string m_ReplyTarget;
    unsigned int m_War3Version; // custom warcraft 3 version for PvPGN users
...
public:
    CBNET(CGProxy *nGProxy, MainGUI *p_mainGUI, string nServer, string nBNLSServer, uint16_t nBNLSPort, uint32_t nBNLSWardenCookie, string nCDKeyROC, string nCDKeyTFT, string nCountryAbbrev, string nCountry, string nUserName, string nUserPassword, string nFirstChannel, unsigned char nWar3Version, BYTEARRAY nEXEVersion, BYTEARRAY nEXEVersionHash, string nPasswordHashType, uint32_t nMaxMessageLength);
    ~CBNET();
    ...
    bool GetListPublicGames()
    {
        return m_ListPublicGames;
    }

    string GetPublicGameFilter()
    {
        return m_PublicGameFilter;
    }

    string GetSearchGameName()
    {
        return m_SearchGameName;
    }

    string GetReplyTarget()
    {
        return m_ReplyTarget;
    }
    ...
    void SetListPublicGames(bool nListPublicGames)
    {
        m_ListPublicGames = nListPublicGames;
    }

    void SetPublicGameFilter(string nPublicGameFilter)
    {
        m_PublicGameFilter = nPublicGameFilter;
    }
