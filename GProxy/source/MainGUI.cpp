...
bool MainGUI::onInputTextAreaKeyPressed(QKeyEvent* event) {
    if (event->key() == Qt::Key_Enter || event->key() == Qt::Key_Return) {
        ...
        if (event->modifiers() != Qt::ShiftModifier) {
            // TO MAKE THE CURSOR GO TO THE END 
            QTextCursor tc = widget.inputTextArea->textCursor();
            tc.setPosition(widget.inputTextArea->document()->characterCount());
            widget.inputTextArea->setTextCursor(tc);
