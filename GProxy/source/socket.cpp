...
void CTCPSocket :: DoRecv( fd_set *fd )
{
	...
	if( FD_ISSET( m_Socket, fd ) )
	{
		...
		if( c == SOCKET_ERROR && GetLastError( ) != EWOULDBLOCK )
		{
			...
			m_HasError = true;
			m_Error = GetLastError( );
			// CONSOLE_PrintQString( "[TCPSOCKET] error (recv) - "
//                                + QString::fromStdString(GetErrorString()));
			return;
		}
		else if( c == 0 )
		{
			...
			// CONSOLE_PrintQString( QString("[TCPSOCKET] closed by remote host") );
			m_Connected = false;
		}
		else if( c > 0 )
		{
...
void CTCPClient :: Connect( string localaddress, string address, uint16_t port )
{
	...
	if( !HostInfo )
	{
		m_HasError = true;
		// m_Error = h_error;
		// CONSOLE_PrintQString( QString("[TCPCLIENT] error (gethostbyname)") );
		return;
	}
...
bool CUDPSocket :: SendTo( string address, uint16_t port, BYTEARRAY message )
{
	...
	if( !HostInfo )
	{
		m_HasError = true;
		// m_Error = h_error;
		// CONSOLE_PrintQString( QString("[UDPSOCKET] error (gethostbyname)") );
		return false;
	}
...
void CUDPSocket :: SetBroadcastTarget( string subnet )
{
	if( subnet.empty( ) )
	{
		// CONSOLE_PrintQString( QString("[UDPSOCKET] using default broadcast target") );
		m_BroadcastTarget.s_addr = INADDR_BROADCAST;
	}
	else
	{
		...
		{
			// CONSOLE_PrintQString( QString("[UDPSOCKET] invalid broadcast target, using default broadcast target") );
			m_BroadcastTarget.s_addr = INADDR_BROADCAST;
		}
	}
}
